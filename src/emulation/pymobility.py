...
def residual_time(mean, delta, shape=(1,)):
    ...
    if delta != 0.0:
        ...
        residual[np.logical_not(
            case_1_u)] = t2-np.sqrt((1.-u[np.logical_not(case_1_u)])*(t2*t2 - t1*t1))
    else:
        residual = u*mean
    return residual
...
def init_random_waypoint(nr_nodes, dimensions,
    ...
    speed_mean, speed_delta = (speed_low+speed_high) / \
        2., (speed_high-speed_low)/2.
    pause_mean, pause_delta = (pause_low+pause_high) / \
        2., (pause_high-pause_low)/2.
    ...
    for i in range(nr_nodes):
        ...
        while True:
            ...
            if rand() < q0:
                ...
                r = np.sqrt(np.sum((z2 - z1) ** 2) /
                            np.sum(np.array(dimensions) ** 2))
                if rand() < r:
                    moving[i] = 1.
                    break
...
class RandomWaypoint(object):
    ...
    def __iter__(self):
        ...
        if self.init_stationary:
            ...
            positions, waypoints, velocity, wt = init_random_waypoint(self.nr_nodes, self.dimensions, MIN_V, MAX_V, wt_min,
                                                                      (self.wt_max if self.wt_max is not None else 0.))
        else:

            NODES = np.arange(self.nr_nodes)
            positions = U(np.zeros(ndim), np.array(
                self.dimensions), np.dstack((NODES,)*ndim)[0])
            waypoints = U(np.zeros(ndim), np.array(
                self.dimensions), np.dstack((NODES,)*ndim)[0])
        ...
        while True:
            ...
            if arrived.size > 0:
                waypoints[arrived] = U(np.zeros(ndim), np.array(
                    self.dimensions), np.zeros((arrived.size, ndim)))
                velocity[arrived] = U(MIN_V, MAX_V, arrived)

                new_direction = waypoints[arrived] - positions[arrived]
                direction[arrived] = new_direction / \
                    np.linalg.norm(new_direction, axis=1)[:, np.newaxis]
...
class StochasticWalk(object):
    ...
    def __iter__(self):
        ...
        xy = U(np.zeros(ndim), np.array(self.dimensions),
...
class RandomDirection(StochasticWalk):
    ...
    def __init__(self, nr_nodes, dimensions, wt_max=None, velocity=(0.1, 1.), border_policy='reflect'):
        ...
        def FL_DISTR(SAMPLES): return U(0, FL_MAX, SAMPLES)
        if wt_max:
            def WT_DISTR(SAMPLES): return U(0, wt_max, SAMPLES)
        else:
            WT_DISTR = None

        def VELOCITY_DISTR(FD): return U(MIN_V, MAX_V, FD)

        StochasticWalk.__init__(self, nr_nodes, dimensions, FL_DISTR,
                                VELOCITY_DISTR, WT_DISTR=WT_DISTR, border_policy=border_policy)
...
class TruncatedLevyWalk(StochasticWalk):
    ...
    def __init__(self, nr_nodes, dimensions, FL_EXP=-2.6, FL_MAX=50., WT_EXP=-1.8, WT_MAX=100., border_policy='reflect'):
        ...
        def FL_DISTR(SAMPLES): return P(FL_EXP, 1., FL_MAX, SAMPLES)
        if WT_EXP and WT_MAX:
            def WT_DISTR(SAMPLES): return P(WT_EXP, 1., WT_MAX, SAMPLES)
        else:
            WT_DISTR = None

        def VELOCITY_DISTR(FD): return np.sqrt(FD)/10.
...
class HeterogeneousTruncatedLevyWalk(StochasticWalk):
    ...
    def __init__(self, nr_nodes, dimensions, WT_EXP=-1.8, WT_MAX=100., FL_EXP=-2.6, FL_MAX=50., border_policy='reflect'):
        ...
        def FL_DISTR(SAMPLES): return rand(len(SAMPLES)) * \
            (FL_MAX[SAMPLES] - FL_MIN[SAMPLES]) + FL_MIN[SAMPLES]
    ...
    while True:
        ...
        for (i, g) in enumerate(groups):
            ...
            x[g] = x_g + g_velocity[i] * g_costheta[i] + \
                aggregation*np.cos(c_theta)
            y[g] = y_g + g_velocity[i] * g_sintheta[i] + \
                aggregation*np.sin(c_theta)
...
def tvc(coords, dimensions, velocity=(0.1, 1.), aggregation=[0.5, 0.], epoch=[100, 100]):
    '''
    ...
    if len(aggregation) != len(epoch):
        raise Exception(
            "The parameters 'aggregation' and 'epoch' should be of same size")
    ...
    while True:
        ...
        if aggr > 0:
            ...
            # update info for arrived groups
            g_arrived = np.where(np.logical_and(
                g_velocity > 0., g_fl <= 0.))[0]
            g_fl = g_fl - g_velocity
            ...
            for (i, g) in enumerate(groups):
                ...
                x[g] = x_g + g_velocity[i] * \
                    g_costheta[i] + aggr*np.cos(c_theta)
                y[g] = y_g + g_velocity[i] * \
                    g_sintheta[i] + aggr*np.sin(c_theta)
