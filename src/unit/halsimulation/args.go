...
func parseArgs() (*Args, error) {
	...
	video := parser.String("", "videos-dir-path", &argparse.Options{Help: "Path to directory of videos to send.", Default: ""})
	ffmpegPath := parser.String("", "ffmpeg-path", &argparse.Options{Help: "Path to ffmpeg executable.", Default: "ffmpeg"})
	audiosDirPath := parser.String("", "audios-dir-path", &argparse.Options{Help: "Path to directory of audios to send. If not provided won't send audio messages.", Default: ""})
